...
#undef ATMEL_EN_SYSFS
#define ATMEL_I2C_RETRY_TIMES 10
#define CONFIG_TOUCHSCREEN_COMPATIBLE_REPORT
#define DBUG_LEVEL 0

#define mXT224_INT 19
#define MXT_TYPE 97
...
static void atmel_ts_work_func(struct work_struct *work)
{
	...
	for (loop_i = 0; loop_i < msg_num; loop_i++) {
		...
		if (report_type >= 0 && report_type < ts->finger_support) {
			...
			ts->finger_data[report_type].id = data[loop_i*9];
			if (data[loop_i * 9 + 1] & 0x20) {//previous reported touch has just removed
			...
			} else if ((data[loop_i * 9 + 1] & 0xC0) && (((ts->finger_pressed >> report_type) & 1) == 0)) {//the touch is present on and just been detected
					...
					if (/*ts->status &&*/ data[loop_i * 9 + 2] >= ts->GCAF_sample) {
						...
						if (ts->GCAF_sample >= 63) {
							...
							i2c_atmel_write_byte_data(ts->client,
								...
								/*ts->config_setting[1].config[1]*/ts->config_setting[0].config[1]);
							i2c_atmel_write_byte_data(ts->client,
		...
		if (!ts->finger_count || ts->face_suppression) {
			...
			for (loop_i = 0, finger_report = 0; loop_i < ts->finger_support; loop_i++) {
				if (((ts->finger_pressed >> loop_i) & 1) == 1) {
						finger_report++;
//				printk("x = %d, y = %d id = %d\n", ts->finger_data[loop_i].x, ts->finger_data[loop_i].y, ts->finger_data[loop_i].id);
							input_report_abs(ts->input_dev, ABS_MT_TRACKING_ID,
								ts->finger_data[loop_i].id);
			 ...
			 const struct i2c_device_id *id)
{
	...
	if(1){
			...
			ret = i2c_atmel_write_byte_data(client, get_object_address(ts, GEN_COMMANDPROCESSOR_T6) + 1, 0x55);//backup settings
			...
			ret = i2c_atmel_write_byte_data(client, get_object_address(ts, GEN_COMMANDPROCESSOR_T6), 0x11);//reset device
			msleep(64);
		}
	...
	input_set_abs_params(ts->input_dev, ABS_MT_TRACKING_ID, 2, 6, 0, 0);
	...
	ret = request_irq(MSM_GPIO_TO_INT(mXT224_INT), atmel_ts_irq_handler, IRQF_TRIGGER_FALLING,client->name, ts);
	if (ret)
...
static int atmel_ts_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	if (ts->power) {
		ret = ts->power(0);
		if (ret < 0)
			printk(KERN_ERR "atmel_ts power off failed\n");
	}
...
static int atmel_ts_resume(struct i2c_client *client)
{
	...
	if (ts->power) {
		ret = ts->power(1);
		if (ret < 0)
			printk(KERN_ERR "atmel_ts power on failed\n");
	}
...
static int __devinit atmel_ts_init(void)
{
	...
	ret = gpio_tlmm_config(GPIO_CFG(mXT224_INT, 0, GPIO_CFG_INPUT,
