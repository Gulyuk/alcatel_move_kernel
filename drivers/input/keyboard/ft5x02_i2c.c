...
//				printk("===x5 = %d,y5 = %d ====\n",event->x5,event->y5);
			...
			/*make sure that the y axis of down position > 320*/

			 else if((event->y1 > 330)&&(down_position_y>320)){

				input_report_abs(data->input_dev, ABS_MT_TOUCH_MAJOR, event->pressure);
				if(event->x1 > 0 && event->x1 < 70){
					input_report_key(data->input_dev, KEY_MENU, 1);
				}else if((event->x1>85) && (event->x1<155)){
					input_report_key(data->input_dev, KEY_SEARCH, 1);
				}else if((event->x1>170) && (event->x1<240)){
